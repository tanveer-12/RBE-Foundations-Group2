...
from openmx_interfaces.srv import EE2JointVel
...
#             # Use a short timeout since this is in the control loop
...
#         1. Limit the magnitude of the joint velocities (max_joint_velocity).
#         2. Limit the resulting position increment (max_position_increment).
...
#             np.array: Limited joint velocities [rad/s]
#         """
#         # 1. Limit individual joint velocities (rad/s)
#         q_dot_limited = np.clip(
#             q_dot,
#             -self.max_joint_velocity,
#             self.max_joint_velocity
...
#         # 2. Limit position increment per step (rad)
#         delta_q = q_dot_limited * self.dt
#         delta_q_limited = np.clip(
#             delta_q,
#             -self.max_position_increment,
...
#         # Log if limits applied
#         if not np.allclose(q_dot, q_dot_final):
#             # Only log if the final velocity is significantly different from the initial
#             if np.linalg.norm(q_dot - q_dot_final) > 1e-4:
#                 self.get_logger().warn(
#                     f'Applied velocity limits: '
#                     f'{np.array2string(q_dot, precision=3)} → '
#                     f'{np.array2string(q_dot_final, precision=3)}'
...
#         Implements: q_ref = q_ref_old + q̇_safe · Δt
...
#         # Note: We can add joint limit checks here if needed, but typically
#         # the robot controller handles hard limits.
...
#         global USING_OPEN_MANIPULATOR
#         if USING_OPEN_MANIPULATOR:
#             # Use OpenManipulator service (SetJointPosition)
#             request = SetJointPosition.Request()
#             request.joint_position.joint_name = self.joint_names
#             request.joint_position.position = q_ref.tolist()
#             # Set path_time slightly longer than dt for smoother motion
#             request.path_time = self.dt * 1.5 
...
#             point = JointTrajectoryPoint()
#             point.positions = q_ref.tolist()
#             # The robot should reach this position after dt seconds
#             point.time_from_start.sec = 0
#             point.time_from_start.nanosec = int(self.dt * 1e9)
...
#     def control_loop(self):
#         """
#         Main control loop - called at fixed rate (self.control_rate Hz).
        
#         Implements the complete velocity control pipeline:
#         1. Check if control should be active (timeout).
#         2. Compute joint velocities via Jacobian (J⁺V).
#         3. Integrate to get position reference (q_ref = q_ref_old + q̇Δt).
#         4. Send to robot.
#         """
#         # Check if we have valid state
#         if self.current_q is None or self.q_ref is None:
...
#         if time_since_cmd > self.cmd_timeout:
#             if self.control_active:
#                 self.get_logger().info('Command timeout - stopping motion')
#                 self.control_active = False
#                 self.desired_ee_velocity = np.zeros(6)
        
#         # If not active, stop and track current position
#         if not self.control_active:
#             # Update reference to current position (prevents jumps on reactivation)
#             self.q_ref = self.current_q.copy()
#             # Ensure the robot is commanded to stop by sending the current position
#             if np.linalg.norm(self.desired_ee_velocity) > 1e-6:
#                 self.send_position_command(self.q_ref)
#             return
...
#         if q_dot_desired is None:
#             # Jacobian failed or timed out - stop motion
#             self.get_logger().error('Failed to compute joint velocities - stopping')
#             self.desired_ee_velocity = np.zeros(6)
#             self.control_active = False
#             return
...
#         if self._log_counter % int(self.control_rate) == 0:  # Every 1 second
#             self.get_logger().info(
#                 f'Control active | '
#                 f'q_dot=[{self.desired_joint_velocity[0]:.3f}, '
#                 f'{self.desired_joint_velocity[1]:.3f}, '
...
#     # Enable debug logging for integration step visualization
#     # rclpy.logging.set_logger_level('incremental_position_controller', rclpy.logging.DEBUG) 
