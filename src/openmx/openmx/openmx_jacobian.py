...
class JacobianVelocityKinematics(Node):
    ...
    def compute_jacobian(self, q):
        ...
        self.get_logger().info(f"DEBUG: J-calculated o_n at {q}: {o_n}")
        # >>> REMOVE AFTER FIXING <<<
        ...
        if abs(q[0]) < 0.1 and abs(q[1]) < 0.1:  # Near home position
            self.get_logger().info("=" * 70)
            self.get_logger().info("JACOBIAN DEBUG AT HOME-ISH POSITION")
            self.get_logger().info(f"q = {q}")
            self.get_logger().info("\nz-axes (rotation axes):")
            self.get_logger().info(f"  z_0 = {z_0}")
            self.get_logger().info(f"  z_1 = {z_1}")
            self.get_logger().info(f"  z_2 = {z_2}")
            self.get_logger().info(f"  z_3 = {z_3}")
            self.get_logger().info("\nPositions:")
            self.get_logger().info(f"  o_n (EE) = {o_n}")
            self.get_logger().info(f"  o1 (J2) = {o1}")
            self.get_logger().info(f"  o2 (J3) = {o2}")
            self.get_logger().info(f"  o3 (J4) = {o3}")
            self.get_logger().info("\nJacobian matrix J:")
            self.get_logger().info(f"  Row 0 (vx): {J[0, :]}")
            self.get_logger().info(f"  Row 1 (vy): {J[1, :]}")
            self.get_logger().info(f"  Row 2 (vz): {J[2, :]}")
            self.get_logger().info("=" * 70)
    ...
    def ee_to_joint_vel_callback(self, request, response):
        ...
        Converts end-effector velocities to joint velocities using:
            q_dot = J^+ * V
        where J^+ is the Moore-Penrose pseudoinverse (computed via SVD).
        ...
        try:
            ...
            V_linear = np.array([request.vx, request.vy, request.vz])
            
            self.get_logger().info(
                f"Inverse velocity request:\n"
                f"  Joint positions (rad): {np.array2string(q, precision=4)}\n"
                f"  EE linear velocity (m/s): [{V_linear[0]:.4f}, {V_linear[1]:.4f}, {V_linear[2]:.4f}]\n"
            )
            ...
            # ================================================================
            # Inverse velocity kinematics using NUMPY'S robust SVD pseudoinverse
            # ================================================================
            J_linear = J[0:3, :]
            # Use np.linalg.pinv for the robust Moore-Penrose pseudoinverse
            # This handles singularities and near-singularities much better
            J_pseudoinv = np.linalg.pinv(J_linear)

            # Inverse velocity kinematics: q_dot = J^+ * V
            q_dot = J_pseudoinv @ V_linear

            # Optional: Calculate condition number as a singularity measure
            # This is a better measure than det(J^T*J) when using pinv.
            s = np.linalg.svd(J_linear, compute_uv=False)
            singularity_measure = s[0] / s[-1] # Condition number (ratio of max to min singular value)
            ...
            self.get_logger().info(
                f"  Joint velocities (rad/s): {np.array2string(q_dot, precision=4)}\n"
                f"  Singularity measure (Condition #): {singularity_measure:.4e}"
            )

            # ================================================================
            # Verification (optional): Check if J * q_dot ≈ V
            # ================================================================
            V_check = J_linear @ q_dot
            velocity_error = np.linalg.norm(V_linear - V_check)
            
            if velocity_error > 1e-3:
                self.get_logger().warn(
                    f"  Velocity verification error: {velocity_error:.6f} m/s\n"
                    f"  This may indicate numerical issues or singularity (Condition # high)"
                )
            else:
                self.get_logger().info(
            ...
            self.get_logger().error(f'Inverse velocity kinematics failed: {str(e)}')
